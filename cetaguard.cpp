...
Cetaguard_receiver::Cetaguard_receiver(Cetaguard_message_interpreter* interpreter_, Cetaguard_exporter_importer* e){
    interpreter = interpreter_;
    bool result = true;
    uint32_t tmp = 0;
    result = result && (e->read_bytes((uint8_t*)&tmp, 4) == 4);
    if (tmp != 0x04030201){
        throw CETAGUARD_IMPORT_FAILED;
    }
    result = result && (e->read_bytes(static_pub_key, PUB_KEY_SIZE) == PUB_KEY_SIZE);
    result = result && (e->read_bytes(static_priv_key, PRIV_KEY_SIZE) == PRIV_KEY_SIZE);
    result = result && (e->read_bytes((uint8_t*)&time_at_last_sync, 4) == 4);
    result = result && (e->read_bytes((uint8_t*)&tmp, 4) == 4);
    clock_source = tmp >> 1 & 0x03;
    in_pairing_mode = tmp & 0x01;
    tmp = 0;
    result = result && (e->read_bytes((uint8_t*)&tmp, 4) == 4);
    for (size_t i = 0; i < tmp; i++){
        Cetaguard_paired_transmitter t;
        result = result && (e->read_bytes((uint8_t*)&t, sizeof(Cetaguard_paired_transmitter)) == sizeof(Cetaguard_paired_transmitter));
        transmitters.push_back(t);
    }
    tmp = 0;
    result = result && e->read_bytes((uint8_t*)&tmp, 4);
    for (size_t i = 0; i < tmp; i++){
        Cetaguard_pending_transmitter p;
        result = result && (e->read_bytes((uint8_t*)&p, sizeof(Cetaguard_pending_transmitter)) == sizeof(Cetaguard_pending_transmitter));
        pending.push_back(p);
    }
    if (!result){
        throw CETAGUARD_IMPORT_FAILED;
...
bool Cetaguard_receiver::export_state(Cetaguard_exporter_importer* e){
    bool result = true;
    uint32_t tmp = 0x04030201;
    result = result && (e->write_bytes((uint8_t*)&tmp, 4) == 4);
    result = result && (e->write_bytes(static_pub_key, PUB_KEY_SIZE) == PUB_KEY_SIZE);
    result = result && (e->write_bytes(static_priv_key, PRIV_KEY_SIZE) == PRIV_KEY_SIZE);
    result = result && (e->write_bytes((uint8_t*)&time_at_last_sync, 4) == 4);
    tmp = clock_source << 1 | in_pairing_mode;
    result = result && (e->write_bytes((uint8_t*)&tmp, 4) == 4);
    tmp = transmitters.size();
    result = result && (e->write_bytes((uint8_t*)&tmp, 4) == 4);
    for (size_t i = 0; i < transmitters.size(); i++){
        result = result && (e->write_bytes((uint8_t*)&transmitters[i], sizeof(Cetaguard_paired_transmitter)) == sizeof(Cetaguard_paired_transmitter));
    }
    tmp = pending.size();
    result = result && e->write_bytes((uint8_t*)&tmp, 4);
    for (size_t i = 0; i < pending.size(); i++){
        result = result && (e->write_bytes((uint8_t*)&pending[i], sizeof(Cetaguard_pending_transmitter)) == sizeof(Cetaguard_pending_transmitter));
    }
    return result;
}
...
bool Cetaguard_receiver::is_time_congruent(uint32_t last_time, uint32_t msg_time, uint32_t last_time_recv, uint32_t recv_time){
	switch (clock_source){
		//So far all options use the same clock congruence algorithm
...
Cetaguard_transmitter::Cetaguard_transmitter(Cetaguard_exporter_importer* e){
    bool result = true;
    uint32_t tmp = 0;
    result = result && (e->read_bytes((uint8_t*)&tmp, 4) == 4);
    if (tmp != 0x05030201){
    ...
    result = result && (e->read_bytes((uint8_t*)&tmp, 4) == 4);
    for (size_t i = 0; i < tmp; i++){
        Cetaguard_paired_receiver t;
        result = result && (e->read_bytes((uint8_t*)&t, sizeof(Cetaguard_paired_receiver)) == sizeof(Cetaguard_paired_receiver));
    ...
    for (size_t i = 0; i < tmp; i++){
        ...
        result = result && (e->read_bytes((uint8_t*)&p, sizeof(Cetaguard_pending_receiver)) == sizeof(Cetaguard_pending_receiver));
        pending.push_back(p);
    }
    if (!result){
        throw CETAGUARD_IMPORT_FAILED;
...
bool Cetaguard_transmitter::export_state(Cetaguard_exporter_importer* e){
    bool result = true;
    uint32_t tmp = 0x05030201;
    result = result && (e->write_bytes((uint8_t*)&tmp, 4) == 4);
    tmp = receivers.size();
    result = result && (e->write_bytes((uint8_t*)&tmp, 4) == 4);
    for (size_t i = 0; i < receivers.size(); i++){
        result = result && (e->write_bytes((uint8_t*)&receivers[i], sizeof(Cetaguard_paired_receiver)) == sizeof(Cetaguard_paired_receiver));
    }
    tmp = pending.size();
    result = result && e->write_bytes((uint8_t*)&tmp, 4);
    for (size_t i = 0; i < pending.size(); i++){
        result = result && (e->write_bytes((uint8_t*)&pending[i], sizeof(Cetaguard_pending_receiver)) == sizeof(Cetaguard_pending_receiver));
    }
    return result;
}
