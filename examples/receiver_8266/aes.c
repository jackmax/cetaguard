...
// The lookup-tables are marked as PROGMEM so they don't take up precious data RAM
...
void AES_key_schedule(uint8_t RoundKey[AES128_SCHEDULE_SIZE], const uint8_t* Key)
{
  unsigned i, j, k;
...
void AES_encrypt(const uint8_t schedule[AES128_SCHEDULE_SIZE], uint8_t buf[AES128_SIZE]){
  ...
  for (round = 1; round < Nr; ++round)
  {
    ...
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(round, state, schedule);
  }
