...
typedef enum
{
    ...
    CETAGUARD_CLOCK_INTERNAL = 1, //Clock source is an internal crystal oscillator, the clock status can be read from a memory-mapped register
    CETAGUARD_CLOCK_LOCAL = 2,    //Clock source is an internal RC oscillator, synchronized with an external crystal oscillator over a bus like I2C or SPI
    CETAGUARD_CLOCK_REMOTE = 3,   //Clock source is an internal RC oscillator, synchronized over the internet
...
public:
    ...
    Cetaguard_status interpret_message(uint8_t* msg_in, size_t length); //WARNING! function may clobber msg_in
    ...
    const uint8_t* get_pairing_secret(Cetaguard_index idx);
    const uint8_t* get_public_key(Cetaguard_index idx);
    void time_was_synchronized();
